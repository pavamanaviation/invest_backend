...
def initiate_nominee_registration(request):
    if request.method == "POST":
        try:
            ...
            # Use correct fields from model
            customer.otp = otp
            # customer.otp_send_type = "nominee"
            customer.changed_on = timezone.now()
            customer.save()

            mobile_number = str(customer.mobile_no).strip()
            if not mobile_number.startswith("91"):
                mobile_number = f"91{mobile_number}"

            send_bulk_sms(mobile_number, otp)
...
def verify_and_save_nominee(request):
    if request.method == "POST":
        try:
            ...
            # Optional session check if needed
            session_customer_id = request.session.get("customer_id")
            if not customer_id or int(customer_id) != int(session_customer_id):
            ...
            # Validate OTP and expiry (2 minutes)
